...
gASRewards = { baker = 0.25, finalizationProof = 0.005, accountCreation = 0.02, chainUpdate = 0.005 }
     ...
     return (
          <>
               ...
               <CModal
                    style={{ height: "90%" }}
                    className="bg-gradient-to-b from-ctp-base to-ctp-crust p-6 overflow-auto mt-10 " scrollable visible={visible} onClose={() => setVisible(false)}>
                    <div>
                         <CModalHeader>
                         ...
                         >
                              <form id="settingsform" className="h-full" onSubmit={handleSubmit}>
                                   <textarea
                                        style={{ height: "65vh" }}
                                        className="w-full h-70" onChange={e => setTomlConfig(e.target.value)} defaultValue={tomlData} />
